...
show_system() {
  ...
  local cpu_total
  cpu_total=$(ps -eo %cpu,comm | grep -E "lto1|mpif90|gfortran|ld" | awk '{s+=$1} END {print s+0}')
...
show_top_errors() {
  echo ""
  echo -e "${CYAN}Top erreurs undefined reference${NC}"
  echo "--------------------------------------------------------------"
  ...
  grep "undefined reference" "$LOG" 2>/dev/null \
    | awk -F'\`' '{print $2}' \
    | sort \
    | uniq -c \
    | sort -nr \
